...
/**
 * Med demons are demons that chase the
 * hero through out the map until they are dead
 *  they have high attack speed but low damage
 *  they move really fast and have average hp
 * @author ross
 *
 */
public class MedDemon extends Demon {
	...
	private String type = "med"; //Easy,Med, or Hard
	...
	public MedDemon(int x, int y) {
		super(x,y);
		pos = new GameCoordinate(x,y,"Med Demon");
		LoadGame.createDemon("M", this);
		this.getImage("Down2");
	}

	public boolean isCloseTo(Hero chara){
		if(Math.abs(this.getCoordinate().getX()-chara.getCoordinate().getX())<16 && Math.abs(this.getCoordinate().getY()-chara.getCoordinate().getY())<16){
			return true;
		}
		return false;
	}
	...
	public void ping(Board b){
		currentState.ping(this,b);
	}

	public void attackPlayer(Hero c) {
		currentState.attackPlayer(c,this.getDamage());
	}
